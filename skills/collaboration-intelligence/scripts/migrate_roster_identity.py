...
# The extractor adds only its token boundaries around ri.SNOWFLAKE_CORE, so the
# grammar has ONE owner; `\d` boundaries reject a mixed-script run whole.
_SNOWFLAKE = re.compile(r"(?<!\d)" + ri.SNOWFLAKE_CORE + r"(?!\d)")
